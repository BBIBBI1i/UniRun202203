...
public class PlayerController : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if(Input.GetMouseButtonDown(0) && jumpCount < 2)
        {
            ...
            // = 점프 직전까지의 힘 또는 속도가 상쇄되거나 합쳐져서 점프 높이가 비일관적으로 되는 현상을 막기 위해서
        ...
        {
            //마우스 왼쪽 버튼에서 손을 뗴는 순간과 속도의 y값이 양수라면 (위로 상승중)
            // 현재 속도를 절반으로 변경
    ...
    private void OnCollisionEnter2D(Collision2D collision)
    {
        ...
        {
            // contacts : 충돌지점들의 정보를 담는 ContactPoint 타입의 데이터를 contacts라는 배열 변수로 제공 받아요.
    ...
    // 단 하나도 isTrigger가 체크가 되어있지 않은 경우
    // 2. OnTrigger 계열 - Enter, Stay, Exit
    // On Trigger계열은 단 하나라도  is Trigger가 체크가 되어 있을 때 사용
}
